...
unsigned int gpio_call(unsigned int pin_number, unsigned int value,
  ...
  const unsigned int num_fields = 32 / field_size;
  const unsigned int reg = base + ((pin_number / num_fields) * 4);
  const unsigned int shift = (pin_number % num_fields) * field_size;
